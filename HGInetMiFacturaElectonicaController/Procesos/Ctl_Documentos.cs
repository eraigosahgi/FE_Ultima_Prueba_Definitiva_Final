...
namespace HGInetMiFacturaElectonicaController.Procesos
{
	...
	public partial class Ctl_Documentos
	{
		...
		public static void ValidarTotales(Factura documento_fac, NotaCredito documento_nc, NotaDebito documento_nd, TipoDocumento tipo_doc, TblEmpresas facturador)
		{
			...
			if (documento != null)
			{
				...
				if (facturador.IntVersionDian == 2)
				{
					...
					//if (!Numero.Tolerancia(total_calculado, decimal.Round(documento.Total, MidpointRounding.AwayFromZero), 10))
					//if (total_calculado != decimal.Round(documento.Total, MidpointRounding.AwayFromZero))
					if (!Numero.Tolerancia(total_calculado, decimal.Round(documento.Total, MidpointRounding.AwayFromZero), 5))
					{
				...
				try
				{
					...
					if (facturador.IntVersionDian == 2)
					{
						...
						if (Docdet.ValorImpuestoConsumo2 > 0 && Docdet.ImpoConsumo2Porcentaje > 0 && Docdet.ProductoGratis == false)
						{
							...
							{
								throw new ApplicationException(string.Format("El campo {0} con valor {1} del detalle no está bien formado, según calculos generados por valor {2}", "ValorImpuestoConsumo2", Docdet.ValorImpuestoConsumo2, impconsumo2_cal));
							}
						}
		...
		public static decimal ValidarDevengados(Devengados devengado, int dias_laborales_periodo, decimal salario_empleado)
		{
			...
			valor_devengado += devengado.SueldoTrabajado + devengado.ApoyoSostenimiento + Comisiones + PagosTerceros + Anticipos + devengado.Dotacion + devengado.Teletrabajo + devengado.BonifRetiro + devengado.Indemnizacion + devengado.Reintegro;
