...
namespace HGInetZonaPagos.Clases
{
	public class VerificarPago
	{
		...
		/// <summary>
		/// Ruta de ejecución del servicio de Zona Virtual.
		/// </summary>
		private string ruta;
		...
		/// <param name="id_tienda">Identificador único del comercio. Este valor es suministrado por Zona Virtual.</param>
		/// <param name="clave">Clave de validación entre el comercio y ZonaPAGOS.</param>
		/// <param name="ruta">Ruta de ejecución del servicio de Zona Virtual.</param>
		public VerificarPago(int id_tienda, string clave, string ruta)
		{
			this.id_tienda = id_tienda;
			this.clave = clave;
			this.ruta = ruta;
		...
		public VerificaPago Verificar(string id_pago)
		{
			try
			{
				...
				// Valida si se envio la ruta para modificar la ejecución del servicio de Zona Virtual
				if (!string.IsNullOrEmpty(this.ruta))
					_cliente.Endpoint.Address = new System.ServiceModel.EndpointAddress(ruta);
