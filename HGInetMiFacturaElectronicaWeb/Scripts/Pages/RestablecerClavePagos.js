...
var DemoApp = angular.module('RestablecerClavePagosApp', ['dx']);
DemoApp.controller('RestablecerClavePagosController', function ($scope, $http) {
	...
	if (id_seguridad != undefined) {
		$http.get('/api/UsuarioPagos?id_seguridad=' + id_seguridad).then(function (response) {

			$scope.RespuestaIdSeguridad = response.data;
			$('#img_cliente').attr("src", "/../Scripts/Images/Terceros/" + response.data + ".png");
		},
		function errorCallback(response) {
		  ...
		  .then((willDelete) => {
		  	if (willDelete) {
		  		window.location.assign("../Login/Pagos.aspx?restablecer=true");
		  	} else {
		  		window.location.assign("../Login/Pagos.aspx");
		  	}
		  });
		});
	} else {
	...
	$scope.onFormSubmit = function (e) {
		...
		if (id_seguridad == undefined) {
			...
			$http.post('/api/UsuarioPagos?' + data).then(function (response) {
				...
				swal({
					...
					window.location.assign("../Login/Pagos.aspx?serial=" + response.data);
				});
