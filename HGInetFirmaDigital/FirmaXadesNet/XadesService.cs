...
namespace FirmaXadesNet
{
	...
	public class XadesService
	{
		...
		public SignatureDocument Sign(Stream input, SignatureParameters parameters, XmlDocument xmlDocument = null)
		{
			...
			switch (parameters.SignaturePackaging)
			{
				...
				case SignaturePackaging.ENVELOPED:
					...
					if(xmlDocument == null)
						xmlDocument = XMLUtil.LoadDocument(input);
		...
		private void SetSignatureId(XadesSignedXml xadesSignedXml)
		{
			...
			//jc  xadesSignedXml.Signature.Id = "Signature-" + id;    LINEA ORIGINAL
			//jc  xadesSignedXml.SignatureValueId = "SignatureValue-" + id;    LINEA ORIGINAL
			// xadesSignedXml.Signature.Id = "xmldsig-fc159e98-be84-4acf-a401-f4555b6c3c6a";// JC
			xadesSignedXml.Signature.Id = "xmldsig-ab2df1fb-1819-413d-8b8c-79e9ed75638a"; //jc
			xadesSignedXml.SignatureValueId = xadesSignedXml.Signature.Id + "-sigvalue";  //JC
		...
		private void SetSignatureDestination(SignatureDocument sigDocument, SignatureXPathExpression destination)
		{
			...
			if (nodo == null)
			{
				throw new Exception("Elemento no encontrado " + destination.XPathExpression);
			}
		...
		private void SetContentEnveloped(SignatureDocument sigDocument, XmlDocument xmlDocument)
		{
			...
			//_refContent.Id = "Reference-" + Guid.NewGuid().ToString(); ORIGINAL
			//_refContent.Id = "xmldsig-fc159e98-be84-4acf-a401-f4555b6c3c6a-ref0"; jc
		...
		private void AddXadesInfo(SignatureDocument sigDocument, SignatureParameters parameters)
		{
			...
			//quite esto       xadesObject.Id = "XadesObjectId-" + Guid.NewGuid().ToString();
			//quite esto       xadesObject.QualifyingProperties.Id = "QualifyingProperties-" + Guid.NewGuid().ToString();
		...
		private void AddCertificateInfo(SignatureDocument sigDocument, SignatureParameters parameters)
		{
			...
			KeyInfo keyInfo = new KeyInfo();
			//keyInfo.Id = "KeyInfoId-" + sigDocument.XadesSignature.Signature.Id; ORIGINAL
			//keyInfo.Id = sigDocument.XadesSignature.Signature.Id + "-keyinfo"; //jc
			keyInfo.Id = "KeyInfo"; //jc

			//Valida si debe agregar informacion y depende de la version de la DIAN
			if (parameters.cKeyInfoDataAdditional == false)
			{
			...
			{
				KeyInfoX509Data data_aditional = new KeyInfoX509Data();
				data_aditional.AddCertificate((X509Certificate)parameters.Signer.Certificate);
				data_aditional.AddSubjectName(parameters.Signer.Certificate.SubjectName.Name);
				data_aditional.AddIssuerSerial(parameters.Signer.Certificate.Issuer, parameters.Signer.Certificate.SerialNumber);

				keyInfo.AddClause(data_aditional);
			}

			//keyInfo.AddClause(new RSAKeyValue((RSA)parameters.Signer.SigningKey)); // en el formato colombiano esta linea no deberia ir evaluar si vi o no va
				   ...
				   UnsignedSignatureProperties unsignedSignatureProperties, SignatureParameters parameters)
		{
			...
			//signedSignatureProperties.SigningCertificate.CertCollection.Add(cert); //esto es el original
			//agruegue como prueba ME PARECE QUE ACA SE DEBERIA AGREGAR SI QUIERO VARIOS CERTS JOSE CARLOS
			// signedSignatureProperties.SigningCertificate.CertCollection.Add(cert);
			///////////////////////////////
			string certPath = parameters.cNombreCertificado;
			string certPass = parameters.cClaveCertificado;
			X509Certificate2Collection collection = new X509Certificate2Collection();

			if (parameters.cdesdealmacen == "SI")
			{
				/* X509Chain chain = new X509Chain();
				   chain.Build(parameters.certificadoalmacen); 
				   for (int i = 0; i < chain.ChainElements.Count; i++)
					{
						Microsoft.Xades.Cert cert1;
						cert1 = new Cert();
						if (parameters.DatoIssuername == "")
						{
							cert1.IssuerSerial.X509IssuerName = chain.ChainElements[i].Certificate.Issuer;
						...
						else
						{
							if (i == 0)
							{
								cert1.IssuerSerial.X509IssuerName = parameters.DatoIssuername0;
							}
							if (i == 1)
							{
								cert1.IssuerSerial.X509IssuerName = parameters.DatoIssuername1;
							}
							if (i == 2)
							{
								cert1.IssuerSerial.X509IssuerName = parameters.DatoIssuername;
							}
						}
						cert1.IssuerSerial.X509SerialNumber = X509Certificate2Extensions.GetSerialNumberAsDecimalString(chain.ChainElements[i].Certificate);
						DigestUtil.SetCertDigest(chain.ChainElements[i].Certificate.GetRawCertData(), parameters.DigestMethod, cert1.CertDigest);
						signedSignatureProperties.SigningCertificate.CertCollection.Add(cert1);
				   } */

				//collection.Import(certPath, certPass, X509KeyStorageFlags.PersistKeySet);
				collection.Import(parameters.certificadoalmacen.GetRawCertData(), parameters.cClaveCertificado, X509KeyStorageFlags.PersistKeySet);
				int ncontador = 0;
				foreach (X509Certificate2 certRoot in collection)
				{
					...
					if (ncontador == 3)
					{
						Microsoft.Xades.Cert cert1;
						cert1 = new Cert();
						cert1.IssuerSerial.X509IssuerName = parameters.DatoIssuername == "" ? certRoot.Issuer : parameters.DatoIssuername;
						cert1.IssuerSerial.X509SerialNumber = X509Certificate2Extensions.GetSerialNumberAsDecimalString(certRoot);
						DigestUtil.SetCertDigest(certRoot.GetRawCertData(), parameters.DigestMethod, cert1.CertDigest);
						signedSignatureProperties.SigningCertificate.CertCollection.Add(cert1);
					}
				}

				ncontador = 0;
				foreach (X509Certificate2 certSub in collection)
				{
					ncontador = ncontador + 1;
					if (ncontador == 2)
					{
						Microsoft.Xades.Cert cert1;
						cert1 = new Cert();
						cert1.IssuerSerial.X509IssuerName = parameters.DatoIssuername1 == "" ? certSub.Issuer : parameters.DatoIssuername1;
						cert1.IssuerSerial.X509SerialNumber = X509Certificate2Extensions.GetSerialNumberAsDecimalString(certSub);
						DigestUtil.SetCertDigest(certSub.GetRawCertData(), parameters.DigestMethod, cert1.CertDigest);
						signedSignatureProperties.SigningCertificate.CertCollection.Add(cert1);
					}
				}
				ncontador = 0;
				foreach (X509Certificate2 certPrin in collection)
				{
					ncontador = ncontador + 1;
					if (ncontador == 1)
					{
						Microsoft.Xades.Cert cert1;
						cert1 = new Cert();
						cert1.IssuerSerial.X509IssuerName = parameters.DatoIssuername0 == "" ? certPrin.Issuer : parameters.DatoIssuername0;
						cert1.IssuerSerial.X509SerialNumber = X509Certificate2Extensions.GetSerialNumberAsDecimalString(certPrin);
						DigestUtil.SetCertDigest(certPrin.GetRawCertData(), parameters.DigestMethod, cert1.CertDigest);
						signedSignatureProperties.SigningCertificate.CertCollection.Add(cert1);
					}
				}
			...
			{
				collection.Import(certPath, certPass, X509KeyStorageFlags.PersistKeySet) ;
				//collection.Import(parameters.certificadoalmacen.GetRawCertData(), "", X509KeyStorageFlags.PersistKeySet);
				int ncontador = 0;
				foreach (X509Certificate2 certRoot in collection )
				{
					ncontador = ncontador + 1;
			...
			if (parameters.SignaturePolicyInfo != null)
			{
				if (!string.IsNullOrEmpty(parameters.SignaturePolicyInfo.PolicyIdentifier))
				{
					...
					//////////////////jose carlos agruegue esta linea
					signedSignatureProperties.SignaturePolicyIdentifier.SignaturePolicyId.SigPolicyId.Description = parameters.cDescripcionPolicy ; //lo agruegue para probarjosecaarlos
				}
			...
			{
				//comente todo esto en el original estaba descomentado  en el formado colombiano no va esto
