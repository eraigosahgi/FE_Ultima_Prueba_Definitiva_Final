...
{
	public class ClienteRest<T> where T : class
	{
		...
		public ClienteRest(string apiUrl, int T_Contenido, string Token)
		{
		...
		private HttpWebRequest SetupClient(int Motodo)
		{
			...
			client.Method = Enumeracion.GetDescription(Enumeracion.GetEnumObjectByValue<Peticion>(Motodo));
			client.ContentType = Enumeracion.GetDescription(Enumeracion.GetEnumObjectByValue<TipoContenido>(_T_Contenido));
			if (_Token != "")
		...
		public T GET()
		{
			...
			try
			{
				HttpWebRequest request = SetupClient(Peticion.GET.GetHashCode());
		...
		public T POST(object parametros)
		{
			...
			try
			{
				...
				HttpWebRequest request = SetupClient(Peticion.POST.GetHashCode());
		...
		public T PUT(List<parametros> parametros)
		{
			...
			try
			{
				HttpWebRequest request = SetupClient(Peticion.PUT.GetHashCode());
