...
namespace LibreriaGlobalHGInet.RegistroLog
{
	public class RegistroLog
	{
		...
		public static void EscribirLog(Exception e, MensajeCategoria categoria_mensaje, MensajeTipo tipo_mensaje, MensajeAccion accion_mensaje, string error_custom="")
		{
			...
			try
			{
				...
				try
				{
					...
					directorio = string.Format("{0}{1}{2}\\", Directorio.ObtenerDirectorioRaiz(), directorio_logs, Fecha.GetFecha().ToString(Fecha.formato_fecha_archivo));

					Directorio.CrearDirectorio(directorio);

					directorio = string.Format("{0}{1}\\", directorio_logs, Fecha.GetFecha().ToString(Fecha.formato_fecha_archivo));

					string ruta_xml = string.Format("{0}{1}.json", directorio, categoria_mensaje);

					// Cargar Archivo XML ya existe
					logXml = Json.ObtenerLog(ruta_xml);
				...
				if (e != null)
				{
					...
					if (st.FrameCount > 0)
					{
						...
						try
						{
							...
							// Obtener Metodo en la pila
							detalleTrace.Metodo = traceString.ToString();
					...
					// Fecha y Accion
					registroLog.Fecha = Funciones.Fecha.GetFecha();
		...
		public static void Guardar(string archivo, string carpeta, string mensaje, string mensaje_adicional)
		{
			try
			{
				string filepath = string.Format(@"{0}\logs\{1}\", Directorio.ObtenerDirectorioRaiz(), carpeta);
				...
				filepath = filepath + DateTime.Today.ToString(Fecha.formato_fecha_java) + "-" + archivo + ".txt";
				...
				{
					string fecha = Fecha.GetFecha().ToString(Fecha.formato_fecha_hora_completa);
					sw.WriteLine(fecha);
