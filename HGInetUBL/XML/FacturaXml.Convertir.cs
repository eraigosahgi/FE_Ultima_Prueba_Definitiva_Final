...
namespace HGInetUBL
{
    public partial class FacturaXML
    {
        ...
        public static Factura Convertir(InvoiceType factura_ubl)
        {
            ...
            foreach (XmlNode item in factura_ubl.UBLExtensions[0].ExtensionContent.ChildNodes)
            {
                if (item.LocalName.Equals("InvoiceControl"))
                {
                    foreach (XmlNode item_child in item.ChildNodes)
                    {
                        ...
                        {
                            if (item_child.FirstChild.LocalName.Equals("Prefix"))
                            {
                                factura_obj.Prefijo = item_child.FirstChild.InnerText;
                            }
                            else
                            {
            ...
            List<FormatoCampo> lista_notas = new List<FormatoCampo>();

            for (int i = 1; i < factura_ubl.Note.Length; i++)
            {
                FormatoCampo formato_campo = new FormatoCampo();

                formato_campo.Valor = factura_ubl.Note[i].Value;
                formato_campo.Ubicacion = "Campo" + i;
                formato_campo.Descripcion = i.ToString();

                lista_notas.Add(formato_campo);
            }

            documento_formato.CamposPredeterminados = lista_notas;
