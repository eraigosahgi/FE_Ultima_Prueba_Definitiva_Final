	...
	public class Cl_Dms
	{
		...
		public static string ObtenerCarpetaPrincipal(string directorio_base, string nit)
		{
			...
			if (string.IsNullOrWhiteSpace(directorio_base))
				ruta = ruta.Replace("{ruta_app}", Cl_Directorio.ObtenerDirectorioRaiz());
			else
			{
		...
		public static string ObtenerCarpeta(string directorio_base, string nit, string compania, int empresa, DmsTipo tipo_archivo, bool crear_carpeta = false)
		{
			...
			switch (tipo_archivo)
			{
				...
				case DmsTipo.PuntosColombiaResponse:
					...
					break;
				case DmsTipo.Temporales:
					carpeta_completa = string.Format(@"{0}{1}\{2}\{3}", ruta_principal, carpeta_cia, carpeta_emp, RecursoDms.CarpetaTemporales);
					break;
				default:
					return "";
			}
			...
			if (crear_carpeta)
				Cl_Directorio.CrearDirectorio(carpeta_completa);
